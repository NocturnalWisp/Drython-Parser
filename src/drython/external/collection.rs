use crate::drython::types::{RegisteredFunction, RegisteredVariable};
use crate::drython::types::Token;
use super::{expect, IsToken};

pub fn register_functs(functions: &mut Vec<RegisteredFunction>)
{
    functions.push(("push_collection".to_string(), Some(Box::new(add_collection))));
    functions.push(("remove_collection".to_string(), Some(Box::new(remove_collection))));
}
...
fn add_collection(mut args: Vec<Token>) -> Result<Option<Token>, String>
{
...
fn remove_collection(mut args: Vec<Token>) -> Result<Option<Token>, String>
{
